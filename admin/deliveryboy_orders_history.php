<?php include('header.php')?>
 ...
 <?php 
$bannerId=isset($_REQUEST['edit_id'])?$dbf->checkSqlInjection($_REQUEST['edit_id']):0;
?>
...
$AddresOfUser=$dbf->fetchSingle("address",'*',"address_id='$Morder[address_id]'");
 ...
 $ordername=$products['product_name'].','.implode(',',$Arry_Vari);
                                                  $message .= '<tr>
                                                    ...
                                                    <td> '.$Morder[qty].' </td>
                                                    <td> Rs. '.number_format($Morder[price],'2').' </td>
                                                  </tr>';
                                              }
                                               $message .= '</tbody>
                                                <tfoot>
                                                  ...
                                                  <tr>
                                                    ...
                                                    <td>Rs. '.number_format($sub_total+$Morder[shipping_charge],'2').'</td>
                                                  </tr>
                                                </tfoot>
                                              </table>
                                            </div>
      ...
      <div class="box">
            ...
            <div class="row">
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php 
                  $i=1;
                  foreach($dbf->fetchOrder("orders","d_boy_id='$bannerId'","created_date DESC","","order_id") as $Order){
           ...
           $Dboy=$dbf->fetchSingle("user",'full_name',"id='$Order[d_boy_id]'");
                  ...
                  <td>
                       <?php 
    if($Order['status']=='0'){?>
        <button  class="uk-button uk-button-small uk-button-danger"  uk-toggle="target: #modal-example2<?= $Order['order_id']?>">New Order</button>
   ...
   <?php }?>        
       ...
       <?php }?>
        ...
        <?php }?>
          ...
          <td><b></b></td>
          <td><b><?php echo number_format($prototal['pro_total']+$CardDetails['gift_price']+ $singleorder['shipping_charge']-$singleorder['wallet']-$return_Amnt,2); ?></b></td>
          </tr>
          </table>
              </div>
