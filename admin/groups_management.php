...
<?php 
  ########################## insert  #############################
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='addgroup'){
$groups_name=trim($dbf->checkXssSqlInjection($_REQUEST['groups_name']));
$cntgroup=$dbf->countRows("groups","groups_name='$groups_name'");
if($cntgroup==0){
...
$string="groups_name='$groups_name',groups_image='$fname1',created_date=NOW()";
$dbf->insertSet("groups",$string);
header("Location:groups_management.php?msg=success");
}else{	
header("Location:groups_management.php?msg=exit");
}
}
}
?>
...
if(isset($_REQUEST['operation']) && $_REQUEST['operation']=='delete'){
 $id=$dbf->checkXssSqlInjection($_REQUEST['groups_id']);
	
	$dbf->deleteFromTable("groups","groups_id='$id'");
...
?>
 ...
 <input type="hidden" name="groups_id" id="groups_id" value="">
 </form>
...
<div class="uk-alert-success" uk-alert>
<a class="uk-alert-close" uk-close></a>
<p>Groups Imported Successfully</p>
</div>
 <?php }?>
...
<div class="uk-alert-success" uk-alert>
<a class="uk-alert-close" uk-close></a>
<p>Groups Updated Successfully</p>
</div>
 <?php }?>
...
<p>Groups Already Exit</p>
 ...
 <?php }?>
            ...
            <?php if(in_array('16.1',$Job_assign)){?>
              <h3 class="box-title"><button class="btn btn-primary" data-toggle="modal" data-target="#modal-default-add">Add Groups</button></h3>
            <?php }?>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <label>Enter Groups Name</label>
                <input type="text" class="form-control" name="groups_name"  placeholder="Enter Groups Name" required />
                </div>
              </div>
              <div class="modal-body">
                <div class="form-group">
              ...
              <button type="submit" class="btn btn-primary pull-left " value="addgroup" name="submit">Add Groups</button>
          ...
          <button type="button" class="btn btn-info " data-toggle="modal" data-target="#myModal212">Import Groups</button>
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <?php
					$i=1;
					 foreach($dbf->fetchOrder("groups","","groups_id ASC","","") as $resBanner){
					?>
                <tr>
                  <td><?php echo $i;?></td>
                  <td><?php echo $resBanner['groups_name'];?></td>
                  <td><img src="images/brands/<?php echo $resBanner['groups_image'];?>" width="30"/></td>
                  <?php if(in_array('16.2',$Job_assign)){?>
            <td> <a class="btn btn-social-icon btn-primary" data-toggle="modal" data-target="#modal-default-edit<?php echo $resBanner['groups_id'];?>" ><i class="fa fa-edit"></i></a></td>
                  <?php }?>

<td> <a href="javascript:void(0);" onClick="deleteRecord('<?php echo $resBanner['groups_id'];?>');" class="btn btn-social-icon btn-danger"  ><i class="fa fa-trash"></i></a>
...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='editgroup'){
...
if($_FILES['img']['name']!='' && (($_FILES['img']['type'] == "image/gif") || ($_FILES['img']['type'] == "image/jpeg") || ($_FILES['img']['type'] == "image/pjpeg") || ($_FILES['img']['type'] == "image/png") || ($_FILES['img']['type'] == "image/bmp"))){
	...
	$id=$dbf->checkXssSqlInjection($_REQUEST['groups_id']);
	$groups_name=$dbf->checkXssSqlInjection($_REQUEST['groups_name']);
	
	
	if($fname1!=''){
	$string="groups_name='$groups_name', groups_image='$fname1', created_date=NOW()";
	}else{
		$string="groups_name='$groups_name', created_date=NOW()";
		}
	
	$dbf->updateTable("groups",$string,"groups_id='$id'");
	
	header('Location:groups_management.php?editId='.$id);exit;
}
?>
...
foreach($dbf->fetchOrder("groups","","groups_id ASC","","") as $resBanner){
?>
              <div class="modal fade" id="modal-default-edit<?php echo $resBanner['groups_id'];?>">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              ...
              <div class="modal-body">
                ...
                <div class="form-group">
                           <input type="hidden" class="form-control" name="groups_id"  value="<?php echo $resBanner['groups_id'];?>">
                    <input type="text" class="form-control" name="groups_name" id="country" value="<?php echo $resBanner['groups_name'];?> "/>
                </div>
                 <div class="form-group">
                  <label for="exampleInputEmail1"> Images</label>
                  <input type="file" class="form-control" id="img" name="img" >
                </div>
                <img src="images/brands/<?php echo $resBanner['groups_image'];?>" width="80px">
              </div>
              <div class="modal-footer">
                <button class="btn btn-default pull-left" type="submit" name="submit" value="editgroup">Update </button>
