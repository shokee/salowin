...
if(isset($_REQUEST['operation']) && $_REQUEST['operation']=='AddVariotion'){
	...
	$contents='';
		$string="price_variation_id='$measure',vendor_id='$vendor_id',mrp_price='$mrp_price',sale_price='$sale_price',product_id='$product',qty='$stockqty'";
		...
		<td><input type="text" placeholder="Enter Sale Price" id="SalesPriceId'.$Measures['price_varition_id'].'" value="'.$VariPrie['sale_price'].'"></td>
			<td><input type="text" placeholder="Enter Stock Qty" id="stockqty'.$Measures['price_varition_id'].'" value="'.$VariPrie['qty'].'"></td>	
			<td><button class="btn btn-primary"  onclick="'.((!empty($VariPrie))?"UpdatePriceVari(".$Measures['price_varition_id'].",".$VariPrie['variations_values_id'].")":"AddPrice(".$Measures['price_varition_id'].")").'">'.((!empty($VariPrie))?"Update":"<i class='fa fa-plus' aria-hidden='true'></i>").'</button></td>
			</tr> ';
		 }
		 echo $contents;exit;
   ...
   if(isset($_REQUEST['operation']) && $_REQUEST['operation']=='UpdateVariotion'){
	...
	if($sale_price == '' AND $mrp_price == ''){
	    
	    $dbf->deleteFromTable("variations_values","variations_values_id='$Variation_id'");
	    
	}else{
		$string="mrp_price='$mrp_price',sale_price='$sale_price',qty='$stockqty'";
