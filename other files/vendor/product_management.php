...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='addcountry'){
  ...
  header("Location:product_management.php?msg=success");
  }else{	
  header("Location:product_management.php?msg=exit");
  }
  }
      ...
      <button class="btn btn-primary" data-toggle="modal" data-target="#modal-default-add">Add Your Brand</button>
            ...
            <div class="row">
              <div class="col-md-3">
                ...
                ?>
                <select name="category" id="FillCategory" class="form-control select2" onchange="GetSubCategory()" style="width:100%;">
              ...
              <div class="col-md-3">
                ...
                <select name="Subcategory" class="form-control select2" id="subCategory"  style="width:100%;" onchange="GetSubCategory2(this.value)">
 ...
 function UpdatePriceVari(Measure_id,Variation_id){
   ...
   if(!sale_price){
    ...
    if(parseFloat(mrp_price)>parseFloat(sale_price)){
	   var url="getAjax.php";
 $.post(url,{"operation":"UpdateVariotion","measure":price_vari,"mrp_price":mrp_price,"sale_price":sale_price,"vendor_id":<?= $profileuserid?>,"product":product,"Variation_id":Variation_id},function(res){
  // alert(res);
  $('#NewPrice').html(res);
});
    }else{
...
function GetAllTable(){
  var Subcateid2 = $('#Subcateid2').val();
  var subCategory = $('#subCategory').val();
  var Category = $('#FillCategory').val();
    var NUmRow = $('#SelectEntity').val();
    $.post('ProdAjax.php',{'num_row':NUmRow,"pagenum":0,"Category":Category,"subCategory":subCategory,"Subcateid2":Subcateid2},function(res){
        // alert(res);
        res = res.split('!next!');
...
function Forward(){
  ...
  var Category = $('#FillCategory').val();
   ...
   pagenum = pagenum+1;
   var NUmRow = $('#SelectEntity').val();
    $.post('ProdAjax.php',{'num_row':NUmRow,"pagenum":pagenum,"Category":Category,"subCategory":subCategory,"Subcateid2":Subcateid2},function(res){
        // alert(res);
        res = res.split('!next!');
    $('#DataTables').html(res[0]);
    $('.pagination').html(res[1]);
...
function PageNUm(num){
  ...
  var Category = $('#FillCategory').val();
  var NUmRow = $('#SelectEntity').val();
    $.post('ProdAjax.php',{'num_row':NUmRow,"num_val":num,"Category":Category,"subCategory":subCategory,"Subcateid2":Subcateid2},function(res){
        // alert(res);
        res = res.split('!next!');
    $('#DataTables').html(res[0]);
    $('.pagination').html(res[1]);
