<li class=" trade-mark uk-text-center">
            <div class="uk-card prdct  uk-background-muted">
        ...
        <?php }?> 
            ...
            <a href="single-product-page.php?editId=<?php echo base64_encode($product['product_id']);?>" onclick="RecentlyViews(<?= $product['product_id']?>)" class="uk-text-secondary">
                <h5 class="uk-margin-remove damara" ><?php echo substr($product['product_name'], 0, 50); ?></h5>
                </a>
         ...
         <select class="uk-select variant uk-margin-small-top uk-margin-small-bottom " id="variation_id<?=$product['product_id']?>" onchange="checkAvailablity(this.value,<?=$product['product_id']?>)" name="variation_id" style="font-size:16px;" >
            <?php
 ...
 <?php
     ...
     <a href="login.php" class="uk-button uk-button-primary   uk-border-rounded " style="white-space: nowrap; text-transform: none;"  >Add to cart</a>
      ...
      ?>
                  ...
                  <div class="quantity" id="QtyBox<?=$product['product_id']?>" <?php if($count_cart == 0){?>style="display:none;"<?php }?> <?php if($checkqty['qty'] <= 0 || $checkqty['qty'] == ''){?>style="display:none;"<?php }?>>
