...
<?php foreach($dbf->fetchOrder("attribute","product_id='$product[product_id]' AND vendor_id='$profileuserid'","","","") as $Attir){
...
<tr id="vari<?= $Attir['attribute_id']?>">
	...
	<td><button type="button" class="btn btn-danger" onclick="DelVari(<?= $Attir['attribute_id']?>,<?= $product['product_id']?>)"><i class="glyphicon glyphicon-remove-sign"></i></button></td>
</tr>
<?php }?>
 </table>
            </div>
...
function GetDynamicTextBox(value) {
 ...
 <script>
  function DelVari(arg,arg1){
    // alert(arg1);
	 var conf=confirm("Are you sure want to delete this Record?");
    if(conf){
       document.getElementById('vari'+arg).style.display = "none"; 
	   var url="getAjax.php";
 $.post(url,{"choice":"variDel","vari_id":arg,"product_id":arg1,"vendor_id":<?= $profileuserid?>},function(res){
